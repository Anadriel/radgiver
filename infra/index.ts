...
const sharedWorkloadIdentityPool = pulumi.output(gcp.iam.WorkloadIdentityPool.get("wif-pool", "gha-pool")).apply(pool =>
    pool ? pool : new gcp.iam.WorkloadIdentityPool("wif-pool", {workloadIdentityPoolId: "gha-pool"})
);

// Workload Identity Federation Provider for GitHub
const githubIdentityPoolProvider =  pulumi.output(gcp.iam.WorkloadIdentityPoolProvider.get("wif-pool-provider-github", pulumi.interpolate`${sharedWorkloadIdentityPool.name}/providers/github-provider`)).apply(provider => 
    provider ? provider : new gcp.iam.WorkloadIdentityPoolProvider("wif-pool-provider-github", {
...
telegramBotToken ? secret.createSecretVersion("telegram-bot-token-version", telegramBotTokenSecret, telegramBotToken) : undefined;
