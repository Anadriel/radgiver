...
function serviceAccountMember(
    principal:
        | gcp.serviceaccount.Account
        | [pulumi.Output<gcp.iam.WorkloadIdentityPool>, string]
) {
...
export function bindServiceAccountToRoles(
    name: string,
    principal: gcp.serviceaccount.Account,
    roles: Set<string>
) {
    const iamBindings = setAsSortedArray(roles).map(role => {
        return new gcp.projects.IAMBinding(
            `${name}-${removeRolePrefix(role)}`,
            {
...
export function createServiceAccountIAMBinding(
    name: string,
    principal:
        | gcp.serviceaccount.Account
        | [pulumi.Output<gcp.iam.WorkloadIdentityPool>, string],
    ...
    const iamBindings = setAsSortedArray(roles).map(role => {
        return new gcp.serviceaccount.IAMBinding(
            `${name}-${removeRolePrefix(role)}`,
            {
...
export function createBucketIAMBinding(
    name: string,
    principal: gcp.serviceaccount.Account,
    ...
    const iamBindings = setAsSortedArray(roles).map(role => {
        return new gcp.storage.BucketIAMBinding(
            `${name}-${removeRolePrefix(role)}`,
            {
                bucket: bucket.name,
                role: role,
...
export function createSecretManagerIAMBinding(
    name: string,
    principal: gcp.serviceaccount.Account,
